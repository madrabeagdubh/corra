// cutaway_bog_gen.mjs
// 48x48 cutaway bog -- worked peatland, turf banks, straight drainage channels,
// spoil heaps, exposed black peat face. Human-shaped landscape.
//
// Usage:
//   node tools/map-editor/generators/cutaway_bog_gen.mjs v001
...
const CONFIG = {
  width: 48, height: 48,
  // Drainage channels -- straight east-west cuts
  channelCount:  4,
  channelWidth:  2,
  // Turf bank rows -- dry raised strips between channels
  bankWidth:     4,
  // Spoil heaps -- scattered bog blocks
  spoilDensity:  0.12,
  noiseScale:    0.3,
  exitWidth:     5,
  exitClearDepth: 4,
...
const MUD      = [1379, 1380]
const BANK     = [1380, 1379]  // slightly drier bank surface
const BOG_BLOCK= [217, 218, 219]
const BOG_BUSH = [49, 50, 53]
...
function buildWater(W,H,cfg,rng) {
  ...
  // Straight drainage channels running east-west
  // Evenly spaced with some y-jitter
  const spacing=Math.floor(H/(cfg.channelCount+1))
  for (let c=0;c<cfg.channelCount;c++) {
    const baseY=(c+1)*spacing + Math.floor((rng()-0.5)*4)
    // Channel runs nearly full width with occasional gaps (blocked sections)
    for (let x=0;x<W;x++) {
      // Occasional short blocking -- turf dam
      const blocked=(x>10&&x<14&&c===1)||(x>28&&x<32&&c===2)
      if (blocked) continue
      for (let dy=0;dy<cfg.channelWidth;dy++) {
        const y=baseY+dy
  ...
  // Small irregular pools in bank sections
  for (let p=0;p<6;p++) {
    const px=Math.floor(rng()*(W-6))+3
    const py=Math.floor(rng()*(H-6))+3
    // Only if not already in a channel
    if (water[py][px]) continue
...
function buildBase(water,W,H,cfg) {
  // Identify bank rows (between channels)
  const spacing=Math.floor(H/(cfg.channelCount+1))
  const channelYs=[]
  for (let c=0;c<cfg.channelCount;c++)
    channelYs.push((c+1)*spacing)
  ...
  return Array.from({length:H},(_,y)=>
    Array.from({length:W},(_,x)=>{
      ...
      // Bank rows are slightly drier
      const nearChannel=channelYs.some(cy=>Math.abs(y-cy)<=cfg.bankWidth/2+1)
      if (nearChannel) return (x+y)%2===0?BANK[0]:BANK[1]
...
function buildOverlay(water,W,H,cfg,rng) {
  ...
  // Spoil heaps (bog blocks) on bank sections
  for (let y=1;y<H-1;y++) {
    for (let x=1;x<W-1;x++) {
      if (water[y][x]||layer[y][x]) continue
      const r=rng()
      const n=(noise2D(x*cfg.noiseScale,y*cfg.noiseScale)+1)/2
      if (r<cfg.spoilDensity&&n>0.55) {
        // Cluster bog blocks -- spoil heap feel
        layer[y][x]=BOG_BLOCK[Math.floor(n*BOG_BLOCK.length)]
      } else if (r<cfg.spoilDensity+0.05) {
...
function generate(outputName) {
  ...
  console.log(`\nGenerating ${W}x${H} cutaway bog: "${outputName}"`)

  const water  =buildWater(W,H,cfg,rng)
  const base   =buildBase(water,W,H,cfg)
  ...
  const map={
    ...
    legend:{
      ...
      '49':'large bog bush','50':'medium bog bush','53':'small bog bush',
      '0':'empty'
    },
  ...
  for (let y=0;y<H;y+=step){
    let row=''
    for (let x=0;x<W;x+=step) row+=water[y][x]?'~':'.'
    console.log(row)
  }
}

const outputName=process.argv[2]||'cutaway_bog_default'
generate(outputName)
