...
drawTilemap() {
    ...
    for (let li = 0; li < this.mapData.layers.length; li++) {
      ...
      // ↓ CHANGED: skip layer 0 entirely when perspective mode is on
      if (li === 0 && this.usePerspective) continue

      if (li === 0) {
        // existing flood fill — only runs when usePerspective is false
      ...
      for (let y = 0; y < layer.length; y++) {
        for (let x = 0; x < layer[y].length; x++) {
          const gid = layer[y][x]
          if (!gid) continue
          // ↓ CHANGED: skip layer 0 tiles when perspective mode is on
          if (li === 0 && this.usePerspective) continue
          const img = this.add.image(
            x * TW * SCALE + (TW * SCALE) / 2,
            y * TH * SCALE + (TH * SCALE) / 2,
    ...
    // ↓ ADD at end of drawTilemap() — instantiate renderer after tilemap is set up
  ...
  update(time, delta) {
    if (this.perspectiveGround) this.perspectiveGround.update()   // ← ADD THIS LINE
